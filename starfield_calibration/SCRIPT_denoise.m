...
% remain only one exposure from each sequence
[~, mask] = unique(expSummary.seq_list,'last');
onePerSequence = expSummary(mask, :)

unique_exp = unique(onePerSequence.exp_length);
nb_unique_exp = length(unique_exp);
fprintf('Computing dark frame for every exposure length\n');
% dark frame is median frame of for different scenes
for nunique_exp = 1:nb_unique_exp
    uniqueexpSummary = onePerSequence(unique_exp(nunique_exp) == (onePerSequence.exp_length),:);
    for nfile = 1:min(height(uniqueexpSummary),10)
